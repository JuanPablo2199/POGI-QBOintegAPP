...
from app.services import qbo_create_api
from app.services import getpush_api
...
def qbo_request(request):
    ...
    if not response.ok:
        return HttpResponse(' '.join([response.text, str(response.status_code)]))
    else:
        return HttpResponse(response.text)

def qbo_create(request):
    auth_client = AuthClient(
        settings.CLIENT_ID,
        settings.CLIENT_SECRET,
        settings.REDIRECT_URI,
    ...
    if auth_client.realm_id is None:
        raise ValueError('Realm id not specified.')
    response = qbo_create_api(auth_client.access_token, auth_client.realm_id)
...
def getpush(requests):

    response = getpush_api()
